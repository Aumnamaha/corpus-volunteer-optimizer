...
def detect_nvidia():
    ...
    smi = _run(["nvidia-smi"])
    cuda_ver = "unknown"
    # Newer drivers (610+) use "CUDA UMD Version:", older use "CUDA Version:"
    # Try both, in order of preference (older/standard format first).
    for pattern in [
        r"CUDA Version:\s*([\d\.]+)",
        r"CUDA UMD Version:\s*([\d\.]+)",
        r"CUDA[^:]*Version:\s*([\d\.]+)",  # generic fallback
    ]:
        m = re.search(pattern, smi)
        if m:
            cuda_ver = m.group(1)
            break
